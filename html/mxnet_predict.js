...
_CWRAP_MXPredCreate = Module.cwrap
    ('MXPredCreate',
     ...
     ['string', // const char* symbol_json_str
      ...
      'number', // codeBookMinVal
      'number', // codeBookMaxVal
      'number', // mx_uint num_input_codeBookKeys,
      'number', // const char** input_codeBookKeys,
      'number', // int dev_type      
      'number', // int dev_id
...
function Predictor(modelobj, input_shapes) {
  ...
  codeBookMinVal = modelobj['codeBookMinVal']
  codeBookMaxVal = modelobj['codeBookMaxVal']
  var i=0;
  for (var key in codeBookMinVal) {i=i+1;}
  var ArrCodeBookMinVal = new Float32Array(i);
  var ArrCodeBookMaxVal = new Float32Array(i);     
  var i=0;
  var ptrarr_ArrCodeBookKeys = [];
  for (var key in codeBookMinVal) {
  	ArrCodeBookMinVal[i]=codeBookMinVal	[key];
  	ArrCodeBookMaxVal[i]=codeBookMaxVal	[key];  	
  	
  	var key_buf = Module._malloc(key.length + 1);
    Module.writeStringToMemory(key, key_buf);
    ptrarr_ArrCodeBookKeys.push(key_buf);   
  	i=i+1;
  }
  var ptr_CodeBookKeys = Module._malloc(SIZEOF_POINTER * ptrarr_ArrCodeBookKeys.length);
  for (var i = 0; i < ptrarr_ArrCodeBookKeys.length; ++i) {
    Module.setValue(ptr_CodeBookKeys + i * SIZEOF_POINTER, ptrarr_ArrCodeBookKeys[i], "*");
  }
  ...
  var ArrCodeBookMinValPtr = Module._malloc(ArrCodeBookMinVal.length * ArrCodeBookMinVal.BYTES_PER_ELEMENT);
  var ArrCodeBookMaxValPtr = Module._malloc(ArrCodeBookMaxVal.length * ArrCodeBookMaxVal.BYTES_PER_ELEMENT);
  ...
  Module.HEAPU8.set(new Uint8Array(ArrCodeBookMinVal.buffer), ArrCodeBookMinValPtr);
  Module.HEAPU8.set(new Uint8Array(ArrCodeBookMaxVal.buffer), ArrCodeBookMaxValPtr);
  ...
  _CWRAP_MXPredCreate(symbol_json,
                      ptr_param_bytes,
                      param_bytes.length,
											ArrCodeBookMinValPtr,
                      ArrCodeBookMaxValPtr,
                      ptrarr_ArrCodeBookKeys.length,
                      ptr_CodeBookKeys,
                      1, 0,
  ...
  Module._free(ArrCodeBookMinValPtr);
  Module._free(ArrCodeBookMaxValPtr);  
  Module._free(ptr_input_shape_data);  
  Module._free(ptr_input_shape_indptr);
  for (var i = 0; i < ptrarr_ArrCodeBookKeys.length; ++i) {
    Module._free(ptrarr_ArrCodeBookKeys[i]);
  }
  for (var i = 0; i < ptrarr_input_keys.length; ++i) {
